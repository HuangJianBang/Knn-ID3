...
def plot_node(nodetext, center_pt, parent_pt, node_type):
    """根据信息绘制一个结点"""
    create_plot.axl.annotate(nodetext, xy=parent_pt, 
...
def get_num_leafs(my_tree):
    ...
    num_leafs = 0
    first_str = list(my_tree.keys())[0]  # no surface
    second_dict = my_tree[first_str]#{0:'no', 1:{'flippers': {0: 'no', 1:'yes'}}}
...
def retrieve_tree(i):
    """测试用的代码，测试get_tree_depth(),get_num_leafs"""

    list_of_trees = [{
        'no surface': {0: 'no', 1: {'flippers': {0: 'no', 1: 'yes'}}}},
        {'no surface': {0: 'no', 1: {'flippers': {0: {'head': {0: 'no',1: 'yes'}},
        1: 'no'}}}}]
